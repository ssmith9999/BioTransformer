...
//	}
	 ...
	 *            : A MetabolicReaction
	 * @param smrkMan
	 *            : an instance of the class SMIRKSManager
	 * @param preprocess
...
//		return ChemStructureExplorer.uniquefy(acontainers);
		...
		if(biotransformations != null){
			for(Biotransformation b : biotransformations){
				for(IAtomContainer ac : b.getProducts().atomContainers()){
					...
					if(hMap.containsKey(ikey) && hMap.get(ikey).getProperty("BioSystem").toString() == b.getBioSystemName().toString()) {
						
						for(Object m : ac.getProperties().keySet()){
							properties.put(m,ac.getProperty(m));
						}
						hash_ac = hMap.get(ikey);
						Set<String> r = new HashSet<String>(Arrays.asList(hash_ac.getProperty("Reaction").toString().split("\n")));
						...
						if(annotate){
							...
							if(data != null ){
								ac.setProperty("PUBCHEM_CID", data.get("CID").get(0));
								ac.setProperty("Synonyms", StringUtils.join(data.get("Synonyms"), "\n")   );
							}else{
								ac.setProperty("PUBCHEM_CID", null);
								ac.setProperty("Synonyms", null );
	...
	//					ac.setProperty("Molecular formula", ChemStructureExplorer.getMolecularFormula(ac));
						properties.put("Molecular formula",ChemStructureExplorer.getMolecularFormula(ac));
	...
	//					properties.put("Reactions",customReactionHash.get(b.getReactionType().toString()).getComonName() + 
	//					" (" + customReactionHash.get(b.getReactionType().toString()).getBTRMID() +")");
						metaboliteID++;
						
	//					ac.setProperty("Metabolite ID", "BTM" + String.format("%04d", metaboliteID));
						properties.put(CDKConstants.TITLE, "BTM" + String.format("%05d", metaboliteID));
	//					ac.setProperty(CDKConstants.TITLE, "BTM" + String.format("%05d", metaboliteID));
						properties.put("Metabolite ID", "BTM" + String.format("%05d", metaboliteID));
	//					ac.setProperty("Metabolite ID", "BTM" + String.format("%05d", metaboliteID));
	...
	//				System.err.println(b.getSubstrates().getAtomContainer(0));
					...
					if(hMap.containsKey(b.getSubstrates().getAtomContainer(0).getProperty("InChIKey"))){
						tt = (String) hMap.get(b.getSubstrates().getAtomContainer(0).getProperty("InChIKey")).getProperty(CDKConstants.TITLE);
						if(tt == null){
							tt = (String) hMap.get(b.getSubstrates().getAtomContainer(0).getProperty("InChIKey")).getProperty("Name");
							...
							if(tt == null) {
								...
								if (tt == null ){
									tt = (String) hMap.get(b.getSubstrates().getAtomContainer(0).getProperty("InChIKey")).getProperty("DATABASE_ID");
									
									if(tt == null) {
										tt = (String) hMap.get(b.getSubstrates().getAtomContainer(0).getProperty("InChIKey")).getProperty("DRUGBANK_ID");
	
										if(tt == null){
											tt = (String) hMap.get(b.getSubstrates().getAtomContainer(0).getProperty("InChIKey")).getProperty("Metabolite ID");
	
											if(tt == null){
												tt = (String) b.getSubstrates().getAtomContainer(0).getProperty("$MolName");
											}
										}
									}
						...
						tt = (String) b.getSubstrates().getAtomContainer(0).getProperty(CDKConstants.TITLE);
						if(tt == null){
							...
							if(tt == null) {
								...
								if (tt == null ){
									tt = (String) b.getSubstrates().getAtomContainer(0).getProperty("DATABASE_ID");
									
									if(tt == null) {
										tt = (String) b.getSubstrates().getAtomContainer(0).getProperty("DRUGBANK_ID");
					...
					if(b.getSubstrates().getAtomContainer(0).getProperty("Molecular formula") == null){
						properties.put("Precursor Molecular formula",ChemStructureExplorer.getMolecularFormula(b.getSubstrates().getAtomContainer(0)));
					}
	
					properties.put("BioSystem", b.getBioSystemName());
					properties.put("Precursor ID", tt);
					
					properties.put("Precursor InChI", b.getSubstrates().getAtomContainer(0).getProperty("InChI"));
					properties.put("Precursor InChIKey", b.getSubstrates().getAtomContainer(0).getProperty("InChIKey"));
					properties.put("Precursor ALogP", b.getSubstrates().getAtomContainer(0).getProperty("ALogP"));
					properties.put("Precursor Major Isotope Mass", b.getSubstrates().getAtomContainer(0).getProperty("Major Isotope Mass"));				
				}
	 ...
	 * @param nr_of_steps - specifies the number of steps for the transformations
